...
typedef enum
{
    n_file, p_log, o_data, o_codice, o_partenza, o_arrivo, s_partenza, _fine
} comando_e;

void allocavett(corsa** C,corsa*** Cdata,corsa*** Ccod,corsa*** Cpart,corsa*** Carr,int nr)
{
    *C=(corsa*)malloc(nr*sizeof(corsa));
    *Cdata=(corsa**)malloc(nr*sizeof(corsa*));
    *Ccod=(corsa**)malloc(nr*sizeof(corsa*));
    *Cpart=(corsa**)malloc(nr*sizeof(corsa*));
    *Carr=(corsa**)malloc(nr*sizeof(corsa*));
}
...
comando_e leggiComando(void)
{
    comando_e r;
    char comando[maxl];
    char cmdtable[_fine][maxl]={"nuovofile","stampa","ordinad","ordinac","ordinap","ordinaa","cercapartenza","fine"};
    printf("digita un comando tra: nuovofile,stampa,ordinad,ordinac,ordinap,ordinaa,cercapartenza,fine\n");
    scanf("%s",comando);
    r=n_file;
    strlwr(comando);
...
void selezionaDati(int* nr,corsa* C,comando_e codcmd,int flag[],comando_e lastcmd, corsa* Carr[],corsa* Cpart[],corsa* Ccod[],corsa* Cdata[],FILE** f)
{
    int i;
    char nomefile[maxl];
    char key[maxl];
        ...
        case n_file:
            {
                ...
                *f=fopen(nomefile,"r");
                ...
                fscanf(*f,"%d",nr);
                allocavett(&C,&Cdata,&Ccod,&Cpart,&Carr,*nr);
...
int main()
{
    ...
    f=fopen("log.txt","r");
    corsa* C;
    corsa **Cdata,**Ccod,**Cpart,**Carr;
    fscanf(f,"%d\n",&nr);
    comando_e codcmd=n_file;
    comando_e lastcmd=n_file;
    allocavett(&C,&Cdata,&Ccod,&Cpart,&Carr,nr);
    if(savefile(nr,f,C))
    {
        for(i=0;i<nr;i++)
        ...
        while(codcmd!=_fine)
        {
            codcmd=leggiComando();
            flag[codcmd]=1;
            selezionaDati(&nr,C,codcmd,flag,lastcmd,Carr,Cpart,Ccod,Cdata,&f);
            if((codcmd!=p_log)&&(codcmd!=s_partenza)&&(codcmd!=n_file))   lastcmd=codcmd;
        }
    }
    fclose(f);
    return 0;
